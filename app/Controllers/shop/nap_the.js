                                ...
                                if (cart !== null) {
                                    ...
                                    tab_NapThe.create({ 'uid': client.UID, 'nhaMang': nhaMang, 'menhGia': menhGia, 'maThe': maThe, 'seri': seri, 'time': new Date() }, function(error, create) {
                                        if (!!create) {
                                            ...
                                            let rID = Math.ceil(Math.random() * (9999999 - 1000000) + 1000000);
                                            let sign = config.key + maThe + 'charging' + config.id + rID + seri + nhaMang;
											console.log(sign);
                                            ...
                                            /*if (nhaMang == "Viettel")
                                                type = "vt";
                                            else if (nhaMang == "Mobile")
                                                type = "mb"
                                            else if (nhaMang == "Vinaphone")
                                                type = "vn"*/

                                            let urlCSKH = config.URL;
			//								+ "?apiKey=" + config.apiKey + "&code=" + maThe + "&serial=" + seri + "&type=" + type + "&menhGia=" + nhaMang_data.value + "&requestId=" + cID + "&callback=";
                                            ...
                                            request({
                                                ...
                                                function(err, httpResponse, body) {
                                                    try {
														...
														let data = body;
                                                        if (data['status'] == '99') {
                                                            let mess = 'Nạp thành công ' + nhaMang + ' ' + menh;
                                                            ...
                                                            UserInfo.findOneAndUpdate({ 'id': client.UID }, { $inc: { red: nhan } }, function(err2, user) {
                                                                client.red({ notice: { title: 'THÀNH CÔNG', text: mess, load: false }, user: { red: user.red * 1 + nhan } });
                                                            });
                                                        } else if (data['status'] == '2') {
                                                            client.red({ notice: { title: 'THẤT BẠI', text: 'Nạp thẻ thành công nhưng sai mệnh giá', load: true } });
                                                        } else if (data['status'] == '3') {
                                                            client.red({ notice: { title: 'THẤT BẠI', text: 'Thẻ không đúng!', load: true } });
                                                        } else {
                                                            client.red({ notice: { title: 'THẤT BẠI', text: data["message"], load: false } });
                                                        }
                                                    } catch (e) {
                                                        client.red({ notice: { title: 'THẤT BẠI', text: 'Hệ thống nạp thẻ tạm thời không hoạt động, Vui lòng quay lại sau.!', load: true } });
                                                    }
                                                });
                                        } else {
                                            client.red({ notice: { title: 'BẢO TRÌ', text: 'Hệ thống nạp thẻ tạp thời không hoạt động, vui lòng giữ lại thẻ và quay lại sau.', load: true } });
                                        }
                                    });
                                }
                            });
